...
-   `database:list {database-cluster?}` - List all databases (schemas) in a database cluster

    -   Options: `--json`

-   `database:get {database-cluster?} {database?}` - Get database (schema) details

    -   Options: `--json`

-   `database:create {database-cluster?}` - Create a new database (schema) in a database cluster

    -   Options: `--name=`, `--json`

-   `database:delete {database-cluster?} {database?}` - Delete a database (schema) from a database cluster
    -   Options: `--force`, `--json`
